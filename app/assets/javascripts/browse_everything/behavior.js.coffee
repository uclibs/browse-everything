(($, window) ->
  ...
  class BrowseEverything
    ...
    constructor: (el, options) ->
      ...
      @$dialog = $('<div tabindex="-1" id="browse-everything" class="ev-browser
      modal fade" aria-live="polite" role="dialog"
      aria-labelledby="beModalLabel"></div>').hide().appendTo('body')

      @render


    @state = {} # This state tree (hash) contains all
    # necessary information to render the
    # UI in the dialog/file explorer at any
    # point in time.
    #
    # The methods below update the state tree without
    # causing any side effects in the DOM. State/data
    # updates and rendering are separate processes
    # that occur in two stages of a data flow.
    #
    # The complexity of rendering
    # this data is handled in the render function,
    # which only handles reflecting the structure
    # and contents of the state tree, and nothing
    # else. This is similar to a game loop, the key
    # difference being that instead of rendering
    # each "frame" at an interval of time, the
    # render function is only called when the
    # data representation of the UI state is
    # updated (whether through a user-given event,
    # or the completion of an ajax request).
    #
    # See: http://nicolashery.com/describing-ui-state-with-data/
    # (Ignore the idiotic hat in his portrait)
    ...
    render: (state) ->
      ...
      # The following functions are event callbacks.
      # All of them either mutate the state of the
      # component or send off an AJAX request whose
      # resolution will call a callback that mutates
      # the state of the component. This separates
      # AJAX requests, state updates, and DOM touching.

  # Define the plugin
  $.fn.extend browseEverything: (options, args...) ->
    @each ->
      $this = $(this)
      data = $this.data('browseEverything')

      if !data
        $this.data 'browseEverything', (data = new BrowseEverything(this, options))
